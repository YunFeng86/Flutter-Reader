...
README, benchmark documentation, and OpenSpec baseline artifacts MUST NOT retain placeholder metadata or contradictory statements after a boundary recalibration change is applied.
...
- **THEN** the recalibration work SHALL update that artifact before the change is considered complete

#### Scenario: Repository facts are stated in more than one baseline artifact
- **WHEN** the same project fact is stated across README, benchmark descriptions, configuration comments, or OpenSpec baseline documents
- **THEN** those artifacts SHALL describe the same current reality without conflicting support claims, causes, or status labels
